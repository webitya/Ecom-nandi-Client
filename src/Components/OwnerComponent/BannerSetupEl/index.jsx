...
const BannerSetupEl = () => {
    ...
    const handleSave = async () => {
        try {
            setLoader(true);
            if (selectedFile && dropdownValue) {
                const formData = new FormData();

                formData.append("banner", selectedFile);
                formData.append("type", dropdownValue); // Include dropdown value
                ...
                toast.error("Please select a file and choose a category!");
            }
        } catch (error) {
            console.log(error);
            toast.error(error?.response?.data?.message || "Some server error occurred!");
        } finally {
            setLoader(false);
            handleReset();
        }
        ...
        <div className="p-4 bg-[#f2f2f2]">
            ...
            <div className="h-[80%] py-4 flex flex-col items-center justify-center bg-white rounded-md mb-2">
                ...
                <select
                    ...
                    className="mt-4 w-full max-w-md px-4 py-2 border rounded text-gray-700 outline-none"
                >
            ...
            <div className="w-full min-h-[20%] bg-white rounded-md p-4">
                {!images.length ? (
                    ...
                    <>
                        <h4 className="text-xl font-semibold mb-3">Uploaded Banners</h4>
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            {images.map((image) => (
                                <div
                                    key={image._id}
                                    className="flex flex-col items-center p-4 border border-gray-300 rounded-md"
                                >
                                    <h3 className="text-xs font-semibold capitalize ">{image.bannerType}</h3>
                                    <img
                                        src={image.bannerUrl}
                                        alt={`Image ${image._id}`}
                                        className="w-full h-32 object-contain mb-4 rounded-md"
            ...
            <Modal
                ...
                footer={null}
            >
                <img src={previewUrl} alt="Banner Image" />
            </Modal>
        </div>
    );
};
